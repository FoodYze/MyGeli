# CÓDIGO FINAL INTEGRADO E CORRIGIDO (V3 - CORREÇÃO DE KEYERROR 'NOME')
...
try:
    ...
    if GOOGLE_API_KEY and GOOGLE_API_KEY != "SUA_CHAVE_API_AQUI":
        ...
        print("Log: API do Gemini configurada com sucesso com o modelo 'gemini-1.5-flash'.")
    else:
...
def buscar_inventario_usuario(conexao):
    if not conexao or not conexao.is_connected():
        return []
    inventario = []
    # [CORREÇÃO DEFINITIVA] A coluna 'nome_produto' é renomeada para 'nome' na consulta.
    # Isso corrige o erro KeyError: 'nome'.
    query = "SELECT nome_produto AS nome, quantidade_produto AS quantidade, tipo_volume AS unidade FROM produtos"
    cursor = None
    try:
...
def formatar_resposta_gemini(texto_gemini):
    ...
    padrao = re.compile(
        r'^\s*[\*\-]?\s*(?P<nome>.+)\s*-\s*(?P<quantidade>[\d.,]+)\s*(?P<unidade>[a-zA-ZáéíóúÁÉÍÓÚçÇ\s/]+)\s*-\s*(?P<categoria>.+?)\s*$',
    ...
    for linha in texto_gemini.split('\n'):
        ...
        if match:
            try:
                ...
                if categoria not in CATEGORIAS_COMPRA:
                    categoria = "Outros"

                itens_formatados.append({"nome": nome, "quantidade": quantidade, "unidade": unidade, "categoria": categoria})
...
def recomendar_produtos_com_gemini(inventario, historico, categorias_validas):
    ...
    try:
        # Esta linha agora funciona, pois 'inventario' contém a chave 'nome'
        inventario_str = ", ".join([f"{item['nome']} ({item['quantidade']} {item['unidade']})" for item in inventario]) if inventario else "vazio"
        historico_str = ", ".join(historico) if historico else "vazio"
        categorias_str = ", ".join(categorias_validas)

        prompt = f"""
        Análise de um assistente de gestão de geladeira:
        
        Meu inventário atual de produtos é: {inventario_str}.
        Meu histórico de produtos mais usados em receitas é: {historico_str}.
        
        Com base nesses dados, sugira uma lista de 5 a 8 itens essenciais que eu deveria comprar.
        
        REGRAS IMPORTANTES:
        1. Para cada item, forneça o nome do produto, uma quantidade sugerida e a unidade de medida (ex: Kg, g, Litros, Unidades).
        2. Para cada item, atribua UMA das seguintes categorias: {categorias_str}.
        3. Formate CADA item EXATAMENTE assim: Nome do Produto - Quantidade Unidade - Categoria
        
        Exemplo de resposta:
        * Tomate - 500 g - Hortifruti
        * Leite Integral - 2 Litros - Laticínios
        * Filé de Frango - 1 Kg - Proteínas
        * Couve-flor - 1 Unidade - Hortifruti 
        """
...
def buscar_produtos_sugeridos():
    ...
    if conexao:
        try:
            ...
            if model:
                produtos_sugeridos = recomendar_produtos_com_gemini(inventario_atual, historico_uso, CATEGORIAS_COMPRA)
        finally:
            conexao.close()

    if not produtos_sugeridos:
        messagebox.showinfo("Sugestões Locais", "Não foi possível buscar sugestões da IA. Carregando uma lista de exemplo.")
        return [
            {"nome": "Leite", "quantidade": 2, "unidade": "Litros", "categoria": "Laticínios"},
            {"nome": "Ovos", "quantidade": 12, "unidade": "Unidades", "categoria": "Proteínas"},
            {"nome": "Farinha", "quantidade": 1, "unidade": "Kg", "categoria": "Mercearia"},
            {"nome": "Açúcar", "quantidade": 500, "unidade": "g", "categoria": "Mercearia"},
            {"nome": "Peito de Frango", "quantidade": 1, "unidade": "Kg", "categoria": "Proteínas"},
        ]
...
def carregar_lista_compras():
    ...
    categorias_ordenadas = sorted(produtos_por_categoria.keys(), key=lambda x: CATEGORIAS_COMPRA.index(x) if x in CATEGORIAS_COMPRA else len(CATEGORIAS_COMPRA))
...
def adicionar_item_manual():
    ...
    def adicionar_e_continuar():
        ...
        quantidade_str = qtd_entry.get().strip()
        if not nome or not quantidade_str:
            messagebox.showerror("Erro", "Nome e quantidade são obrigatórios.", parent=dialog)
            return
        novo_item = {"nome": nome.capitalize(), "quantidade": quantidade_str, "unidade": unidade_var.get(), "categoria": categoria_var.get()}
...
if __name__ == "__main__":
    ...
    subtitulo = ctk.CTkLabel(content_frame, text="Sugestões baseadas no seu consumo", font=ctk.CTkFont(size=14), text_color="#666666")
