...
class InventoryApp(ctk.CTk):
    def __init__(self, db_connection):
        ...
        self.measurement_units = ["Unidades", "Kg", "Litros", "Gramas"]
        self.create_widgets()
    ...
    def _add_item_widget(self, name, qty, unit, row_index):
        """ Cria o widget de um item na lista, agora sem a imagem e com alerta de estoque baixo. """
        
        # --- LÓGICA DE ALERTA ---
        # Define a cor baseada na quantidade.
        try:
            numeric_qty = float(qty)
            is_low_stock = False
            
            # Limiares de alerta para cada unidade
            if unit.lower() in ["unidades", "unidade"]:
                if numeric_qty <= 2:
                    is_low_stock = True
            elif unit.lower() == "kg":
                if numeric_qty <= 0.5: # 500g ou menos
                    is_low_stock = True
            elif unit.lower() == "litros":
                if numeric_qty <= 0.5: # 500ml ou menos
                    is_low_stock = True
            elif unit.lower() == "gramas":
                if numeric_qty <= 100: # 100g ou menos
                    is_low_stock = True
            
            item_color = "#FFA07A" if is_low_stock else "#0084FF" # Laranja claro para alerta, azul para normal
            text_color = "#000000" if is_low_stock else "white" # Texto preto no alerta para melhor leitura

        except (ValueError, TypeError):
            item_color = "#0084FF" # Cor padrão se a quantidade não for numérica
            text_color = "white"
        ...
        formatted_qtd = "{:g}".format(float(qty)).replace('.', ',') if qty is not None else "0"
    ...
    def open_remove_item_dialog(self):
        ...
        def _remove_item_action():
            ...
            if not name: messagebox.showerror(title="Erro", message="Selecione um item para remover.", parent=di
